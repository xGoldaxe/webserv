...
namespace preq {
	...
	bool	parse_url( std::string url, std::string & ret_url,
		std::string & path_info, std::string & query_string, size_t max_url_size = MAX_URL_SIZE );
	...
	int parse_request( std::string & data, void (*print_or_store)(std::vector<std::string>,
		std::map<std::string, std::string>, std::string, std::string), size_t max_url_size = MAX_URL_SIZE );
}